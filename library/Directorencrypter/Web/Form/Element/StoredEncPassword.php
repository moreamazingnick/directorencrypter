 ...
 *   
 *   Passwords get wrapped by PREENC() in order to tell the beforeSave to encrypt
 *   it
 *
 * @package Icinga\Module\Director\Web\Form\Element
 */
class StoredEncPassword extends ZfText
{
    ...
    public function setValue($value)
    {
        if (\is_array($value) && isset($value['_value'], $value['_sent'])
            ...
            if ($sentValue !== self::UNCHANGED) {
                $sentValue = sprintf("PREENC(%s)", $sentValue);
                $this->setAttrib('sentValue', $sentValue);
                return parent::setValue((string) $sentValue);
            }
        } else {
            $sentValue = null;
        }
